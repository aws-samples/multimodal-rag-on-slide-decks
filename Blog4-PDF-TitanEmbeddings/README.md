...
- [3_rag_inference.ipynb](Blog4-PDF-TitanEmbeddings/notebooks/3_rag_inference.ipynb) - This notebook uses performs a **Hybrid Search** and uses an _LLM in the loop_ to look up the most relevant data and parse through which data corresponds and gives the most appropriate answer to the user provided question.
...
- [main.py](Blog4-PDF-TitanEmbeddings/notebooks/main.py) - Script to run all the notebooks through a single command. See section on `Running`.

- [config.yaml](Blog4-PDF-TitanEmbeddings/notebooks/config.yml) - contains configuration parameters such as directory path, model information etc. for this solution. ***The pricing information in the [`config.yml`](./config.yml) is subject to change, please always confirm Bedrock pricing from the [_Amazon Bedrock Pricing_](https://aws.amazon.com/bedrock/pricing/) page***.
