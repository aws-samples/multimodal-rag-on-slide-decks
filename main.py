...
def run_notebooks(config_file: str) -> None:
    ...
    logger.info(f"Multimodal enhanced RAG workflow is complete. View results in the local {config['dir']['metrics']} directory.")
...
def main():
    ...
    current_directory = Path(__file__).parent
    config_file_path: str = os.path.join(current_directory, 'config.yaml')
