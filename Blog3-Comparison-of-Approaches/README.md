...
In our testing, both approaches produced <=50% accurate results to the questions in the manifest. Our random selection of slide decks covered a wide variety of industries including retail, healthcare, academic, technology, personal, travel etc. The embeddings were all ingested into a single index. So a generic question like "What are examples of tools that can be used?" would have no additional context. The closest match can be retrieved from a completely different slide deck, hence not matching the accurate response. We will provide a sneakpeek into ways we seek to improve these results in the conclusion below.
